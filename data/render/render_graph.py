...
log_file_path = "/Users/matt/Documents/vwork/1210/log1210_3.txt"
...
with open(log_file_path, 'r') as file:
    for line in file:
        ...
        #     is_data_line = False
        if line.__contains__('MapEngine-renderop:over-ttop-render--over'):
            is_data_line = False
        ...
        if match:
            ...
            if is_data_line:
                ...
                #     csv_data_dict[title] = value
                # else :
                #     csv_data_dict[title] = csv_data_dict[title] + value
                # start ------------
                # if isHead:
                #     csv_headers.append(title)
                # csv_data_dict[title] = value
                ...
                csv_data_dict['renderTime'] = value
...
try:
    ...
    for row in df.itertuples(index=True):
        ...
        total = row.ready + row.loadDataHD + row.renderHD

        openglInit = row.setMat
        total = row.ready + row.loadDataHD + row.renderHD  + openglInit
        ...
        RenderElements = row.skybox+row.plane+row.greenBelt+row.roadbed+row.divider+row.divider_texture\
                         +row.renderPost+row.PainterMgr__render#+row.zebra+row.diversion+row.safetyIsland
        renderScene = row.skybox+row.plane+row.greenBelt+row.roadbed+row.divider+row.divider_texture#+row.zebra+row.diversion+row.safetyIsland
        ...
        renderDetailHD_dict['zebra'] = 0#row.zebra
        renderDetailHD_dict['diversion'] = 0#row.diversion
        renderDetailHD_dict['divider'] = row.divider + row.divider_texture
        renderDetailHD_dict['safetyIsland'] = 0#row.safetyIsland
