...
# 读取CSV文件
# df = pd.read_csv('/Users/matt/Downloads/卡顿问题/firegraph/graphTotal.csv')
# df = pd.read_csv('/Users/matt/Documents/vwork/1209/graphTotal.csv')
df = pd.read_csv('/Users/matt/Documents/vwork/1210/graphTotal_1.csv')

# 将DataFrame转换为字典
data = df.to_dict(orient='list')
# data = df.head(2000).to_dict(orient='list')
...
categories = list(data.keys())[2:]  # 排除 'index'和total 列
...
#     ax.text(i, bottom[i] + value / 2, str(value), ha='center', va='center')

font  = FontProperties(fname='/System/Library/Fonts/PingFang.ttc')
# 添加标签和标题
ax.set_xlabel('数量', fontproperties=font)
ax.set_ylabel('耗时 (us)', fontproperties=font)
ax.set_title('渲染耗时火焰图', fontproperties=font)
ax.legend()
